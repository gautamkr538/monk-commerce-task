-- SQL Schema for Coupon Management System with JOINED Inheritance Strategy
-- Base Coupon Table
CREATE TABLE coupon (id BIGSERIAL PRIMARY KEY,
coupon_code VARCHAR(50) UNIQUE NOT NULL,type VARCHAR(20) NOT NULL,description VARCHAR(500),
is_active BOOLEAN NOT NULL DEFAULT TRUE,expiration_date TIMESTAMP,
created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP);

-- Cart-Wise Coupon Table
CREATE TABLE cart_wise_coupon (id BIGINT PRIMARY KEY REFERENCES coupon(id) ON DELETE CASCADE,
threshold_amount DECIMAL(10,2) NOT NULL,
discount_percentage DECIMAL(5,2),max_discount_amount DECIMAL(10,2));

-- Product-Wise Coupon Table
CREATE TABLE product_wise_coupon (id BIGINT PRIMARY KEY REFERENCES coupon(id) ON DELETE CASCADE,
product_id BIGINT NOT NULL,discount_percentage DECIMAL(5,2),max_discount_per_product DECIMAL(10,2));

-- BxGy Coupon Table
CREATE TABLE bxgy_coupon (id BIGINT PRIMARY KEY REFERENCES coupon(id) ON DELETE CASCADE,
...
-- Get Products Table
CREATE TABLE get_product (id BIGSERIAL PRIMARY KEY,
bxgy_coupon_id BIGINT NOT NULL REFERENCES bxgy_coupon(id) ON DELETE CASCADE,
product_id BIGINT NOT NULL,quantity INTEGER NOT NULL);
